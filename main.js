...
client.on("message", async message => {
  ...
  if (message.content.startsWith(`${prefix}play`)) {
    ...
    message.channel.send("Entre com um comando válido.");
  }
});
...
async function execute(message, serverQueue) {
  ...
  if (!serverQueue) {
    ...
    return message.channel.send(`${song.title} foi adicionado na lista de reprodução!`);
  }
}
...
function skip(message, serverQueue) {
  if (!message.member.voice.channel)
    return message.channel.send(
      "Você precisa estar conectado a um canal de voz para parar a música!"
    );
  if (!serverQueue)
...
function stop(message, serverQueue) {
  ...
  if (!serverQueue)
    return message.channel.send("Não há música que eu pudesse parar!");
